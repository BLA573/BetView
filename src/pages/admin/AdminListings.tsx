...
const AdminListings = () => {
  ...
  const fetchProperties = async () => {
    ...
    let query = supabase
      .from("properties")
      .select("id, title, location, price, mode, type, rooms, images, status, created_at, agency_id")
      .order("created_at", { ascending: false });

    if (filter === "orphan") {
      query = (query as any).is("agency_id", null);
    } else if (filter !== "all") {
    ...
    <AdminLayout>
      <div className="space-y-6">
        ...
        <div className="flex gap-2">
          {["all", "approved", "pending", "rejected", "orphan"].map((f) => (
            <button
              key={f}
              onClick={() => setFilter(f)}
